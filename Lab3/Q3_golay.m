...
%% Gerando o código golay(23, 12):
k = 12;
n = 23;
code_gol = fun_gol(n, k);

%% Gerando info com tamanho k = 12:
u = randi([0 1], 1, Ncw*k);
u_reshaped = reshape(u, Ncw, k);

%% Codificando a info com o código hamming(7, 4):
c_gol_reshaped = mod(u_reshaped*code_gol.g, 2);
c_gol = reshape(c_gol_reshaped, 1, Ncw*n);
...
for EbNo = EbNo_list
    ...
    u_hdd = decodeHDD(b_reshaped, code_gol);
    ...
    u_sdd = decodeSDD(r_reshaped, code_gol);
...
pesos_erro = sum(code_gol.lut.e_hat');
